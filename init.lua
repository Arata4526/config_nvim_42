...
-- terminal vertical à gauche, 30 colonnes, dans le working directory courant
...
-- auto entrer en mode insertion dans le terminal
vim.api.nvim_create_autocmd({ "TermOpen", "BufEnter" }, {
  pattern = "term://*",
  command = "startinsert",
})

-- revenir en normal mode quand tu quittes le terminal
...
function M.line(n)
  ...
  elseif n == 6 then
    return M.textline(
      "By: " .. (vim.g.user42 or "albousqu") .. " <" .. (vim.g.mail42 or "albousqu@student.42.fr") .. ">",
      M.ascii(n)
    )
  elseif n == 8 then
    return M.textline(
      "Created: " .. vim.fn.strftime("%Y/%m/%d %H:%M:%S") .. " by " .. (vim.g.user42 or "albousqu"),
      M.ascii(n)
    )
  elseif n == 9 then
    return M.textline(
      "Updated: " .. vim.fn.strftime("%Y/%m/%d %H:%M:%S") .. " by " .. (vim.g.user42 or "albousqu"),
      M.ascii(n)
    )
  end
end
